...
func writeMapFiles(mapf func(string, string) []KeyValue, params ExampleReply) {
	...
	for i := 0; i < params.NReduce; i++ {
		filename[i] = s.Join([]string{"mr",strconv.Itoa(params.Id),strconv.Itoa(i)},"-")
                //filename[i] = s.Join([]string{"/Users/kalpeshpatel/mit-class/src/mr/data",filename[i]},"/")
	...
	for i := 0; i < params.NReduce; i++ {
		err := os.Rename(tmpfile[i].Name(),s.Join([]string{"/Users/kalpeshpatel/mit-class/src/mr/data",filename[i]},"/"))
		if err != nil {
		log.Fatal(err)
		}
...
//It needs to iterate through all workerID
	...
	for i:= 0;i < length; i++ {
		filename := s.Join([]string{"/Users/kalpeshpatel/mit-class/src/mr/data/mr",strconv.Itoa(params.MapWorkerID[i]),strconv.Itoa(params.Bucket)},"-")
	...
	oname := s.Join([]string{"/Users/kalpeshpatel/mit-class/src/mr/data/mr","out",strconv.Itoa(params.Bucket)},"-")
