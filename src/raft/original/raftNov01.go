	...
	if args.Term < rf.currentTerm { //heartbeat scenario
		...
		if args.LeaderCommit > rf.commitIndex {
			...
			for rf.lastApplied <= rf.commitIndex {
				rf.lastApplied++
				fmt.Println("Applying: ", rf.log[rf.lastApplied])
				rf.myCh <- ApplyMsg{true, rf.log[rf.lastApplied].Command, rf.lastApplied}
			}
			//rf.lastApplied = rf.commitIndex
			**/
...
func (rf *Raft) commitApplier() {
	...
	for i := rf.lastApplied + 1; i <= rf.commitIndex; i++ {
		fmt.Println("Applying Entry: Server:", rf.log[i], rf.me)
		rf.myCh <- ApplyMsg{true, rf.log[i].Command, i}
	}
	rf.lastApplied = rf.commitIndex
}
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if args.Term < rf.currentTerm {
		...
		if args.Term > rf.currentTerm && args.LastLogTerm < rf.log[len(rf.log)-1].Term {
			...
			return
		} else if args.Term == rf.currentTerm && args.LastLogTerm > rf.log[len(rf.log)-1].Term && rf.votedFor == -1 && args.LastLogIndex > len(rf.log)-1 {
			reply.VoteGranted = true
			reply.Term = args.Term
			rf.votedFor = args.CandidateId
			rf.validBeat = true
			...
			//TO DO: need to check if it works under all scenarios
	...
	for {
		...
		if len(rf.log) >= startIndex+1 {
			...
			if wonElection(count, serverCount) {
				rf.commitIndex++
				rf.myCh <- ApplyMsg{true, rf.log[rf.commitIndex].Command, rf.commitIndex}
				fmt.Println("Entry Applied: Leader: Command:", rf.me, rf.log[rf.commitIndex].Command)
				rf.lastApplied = rf.commitIndex
			} else {
				return
			}
