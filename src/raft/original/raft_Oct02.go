...
func (rf *Raft) sendHeartBeat() {
	for i := 0; i < serverCount; i++ {
		if i != rf.me {
			rf.mu.Lock()
			newArgs := AppendEntriesArgs{rf.currentTerm, rf.me}
			rf.mu.Unlock()
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if args.Term < rf.currentTerm {
		...
		rf.validBeat = true
	} else if rf.votedFor == -1 && args.LastLogTerm == rf.log[rf.commitIndex].term && args.LastLogIndex > len(rf.log) {
		reply.VoteGranted = true
		reply.Term = rf.currentTerm
		rf.votedFor = args.CandidateId
		rf.validBeat = true
		fmt.Println("Vote granted: Server: Term: To: ", rf.me, rf.currentTerm, args.CandidateId)
	} else { fmt.Println("Vote note granted")} 
}

	/** else if args.Term > rf.currentTerm { //no need to check voted for
		...
		rf.validBeat = true
		//fmt.Println("Vote granted: Server: Term: To: ", rf.me, rf.currentTerm, args.CandidateId)
		//rf.peerStatus = Follower -- don't chnage until server becomes leader
	} ****/
